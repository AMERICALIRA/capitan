class ProfileController < ApplicationController
    def myprofile
      profile_information current_user
    end
    ...
    #TODO: This need to be move to services
    def profile_information(user)
        @max_total_points = 0
        @max_student_points = 0
        @data = []
        @sprint_index = 0
        @user = user
        ...
        unless @sprints.empty?
            ...
            if !@maximum_points.empty? && (@maximum_points.length == @student_points.length)
                @data = @maximum_points.zip(@student_points).map do |arr|
